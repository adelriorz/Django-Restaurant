...
def order(request):
    ...
    if request.method == 'POST':
        filled_form = PizzaForm(request.POST) #request.FILES for submitting file
        if filled_form.is_valid():
            created_pizza = filled_form.save()
            created_pizza_pk = created_pizza.id
            note = 'Thanks for ordering! Your %s , %s %s pizza its on the way' %(filled_form.cleaned_data['size'],
            filled_form.cleaned_data['topping1'],
            filled_form.cleaned_data['topping2'],)
            filled_form = PizzaForm()
        else: # Error handiling for errors in form
...
def pizzas(request):
    ...
    if filled_multiple_pizza_form.is_valid():
        ...
        print(filled_multiple_pizza_form.cleaned_data['number'])
